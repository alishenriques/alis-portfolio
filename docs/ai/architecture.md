...
### Layout: sidebar drawer, not a persistent sidebar or the command palette

- `src/modules/layout/Layout.tsx`: wraps every page with `ContactDialogProvider` + `TopBar` + the page + `ContactDialog`.
- `TopBar`: slim sticky header — the `Sidebar` drawer trigger, the `Logo`, and `LanguageSwitcher`.
- `Sidebar` (`modules/layout/components/Sidebar`): a hamburger button plus an animated slide-in/out overlay drawer (nav links from `constants/menuConfig.ts`, using next-intl's `Link`), not a permanently-docked sidebar. The drawer stays mounted and slides off-screen via a CSS transform, with `aria-hidden` toggled on the closed `<aside>` for accessibility — tests rely on `aria-hidden` exclusion (`queryByRole` returns null for hidden content), not `toBeVisible()`, which doesn't account for transforms.
...
- `app/[locale]/page.tsx` → `modules/home/Home.tsx`: `Hero` (logo, headline, rich-text subtitle, contact row, tagline strip, decorative `ParallaxPanels`) → `FeatureChips` (6-item value-prop grid) → `Skills` (`modules/skills`, real primary stack rendered with official brand icons from `@icons-pack/react-simple-icons`) → the Projects section (`ProjectCard`, from `shared/components`).
